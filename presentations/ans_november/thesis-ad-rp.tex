...
The integral on the right hand side is the same as in
  Eq.~\eqref{eq:adFicks}, which defined the anisotropic diffusion
...
Substituting Eq.~\eqref{eq:adFicks}, the first moment of the AD
...
low-order AD equations~\eqref{eq:loVol}, \eqref{eq:adFicks}, 
...
Eq.~\eqref{eq:adFicks}, we recover the standard Fick's law:
\begin{equation*}
...
Eq.~\eqref{eq:adFicks} into Eq.~\eqref{eq:loVol}:
\begin{equation*}
\frac{1}{c} \pder{\phi}{t} (\vec{x})
