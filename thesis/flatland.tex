...
In our discussion of flatland boundary conditions, we have essentially shown
that the flatland equivalent of the $W$ function, which we shall call $V$, can
be approximated by
\begin{equation}\label{eq:flatV2}
  V_2(\mu)
  \equiv
  \frac{1}{2} \mu + \frac{1}{\pi}\mu^2 
  \,,
  \quad
  0 < \mu \le 1 \,,
\end{equation}
where $\mu = \abs{\vec{\Omega} \vd \vec{n}}$, and the equation is normalized
so that
\begin{equation*}
  \int_{0}^{\pi} V_2(\sin\omega) \ud\omega = 1 \,.
\end{equation*}
The ``variational'' flatland extrapolation distance is:
\begin{equation*}
 z_0 = \frac{\int_{0}^{\pi} V_2(\sin\omega) \sin\omega \ud\omega}%
  {\int_{0}^{\pi} V_2(\sin\omega) \ud\omega}
  = \frac{\pi}{8} + \frac{4}{3\pi}
  \approx
0.8171\,.
\end{equation*}

We also found the flatland Marshak approximation to be:
\begin{equation}\label{eq:flatV1}
  V_1(\mu) \equiv \pi \mu \,,
  \quad 0 < \mu \le 1 \,,
\end{equation}
giving the extrapolation distance
\begin{equation*}
 z_0 = \frac{\int_{0}^{\pi} V_2(\sin\omega) \sin\omega \ud\omega}%
  {\int_{0}^{\pi} V_2(\sin\omega) \ud\omega}
  = \frac{\pi}{4}
...
\begin{equation*}
  \int_{0}^{\pi} V(\sin\omega) \ud\omega = 1 \,.
\end{equation*}
The ``variational'' flatland extrapolation distance is:
\begin{equation*}
  \frac{\int_{0}^{\pi} V_2(\sin\omega) \sin\omega \ud\omega}%
  {\int_{0}^{\pi} V_2(\sin\omega) \ud\omega}
  = \frac{\pi}{8} + \frac{4}{3\pi}
  \approx
0.8171
