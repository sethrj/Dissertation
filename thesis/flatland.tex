...
in the interior of a 3-D problem, the necessary condition was
...
\begin{equation*}
  ...
  \approx 0.7104\,,
\end{equation*}
and the variational approximation gives the extrapolation distance
\begin{equation*}
  z_0 \approx \int_{0}^{1} \mu W_2(\mu) \ud\mu = \tfrac{17}{24}
  \approx 0.7083\,.
\end{equation*}
Similarly, the 3-D Marshak boundary condition used
\begin{equation*}
  W(\mu) \approx W_1(\mu) \equiv 2 \mu \,,
\end{equation*}
which gave the Marshak extrapolation distance
\begin{equation*}
  z_0 \approx \int_{0}^{1} \mu W_1(\mu) \ud\mu = \tfrac{2}{3}
  \approx 0.6667\,.
\end{equation*}

In our discussion of flatland boundary conditions, we have essentially been
investigating flatland equivalent of the $W$ function, which we denote as $V$,
that preserves the interior solution:
...
\end{equation}
The ``true'' function $V$ could be calculated, for example, by using a
...
&= 
 ...
 \vd \grad \phi(\vec{x},t) \,.
\end{align*}
Multiplying by $2\pi$, we obtain the flatland anisotropic diffusion boundary
condition:
...
Eq.~\eqref{eq:adBoundaryIntegral} to be
\begin{equation}\label{eq:flatAdBoundaryIntegral}
  \vec{d}(\vec{x}) = -\pi\int_{\vec{\Omega}\vd\vec{n} < 0}
...
\begin{equation*}
  ...
  \vd \grad \phi(\vec{x},t) \,,
\end{equation*}
i.e., under the assumption that $f$ is rotationally invariant about the
boundary normal,
...
\\
&= \frac{1}{2\sigma} \left( \int_{\vec{\Omega}\vd\vec{n} < 0}
  V(\abs{\vec{\Omega}\vd\vec{n}})
...
&=
\phi(\vec{x},t)
+ 2 \left[ -\pi\int_{\vec{\Omega}\vd\vec{n} < 0}
...
\begin{align*}
  ...
  \vec{\Omega} f(\vec{x},\vec{\Omega}) \ud\Omega
  \\
  &= \frac{\pi}{2} \int_{\vec{\Omega}\vd\vec{n} < 0} (\vec{\Omega}\vd\vec{n})
  \vec{\Omega} f(\vec{x},\vec{\Omega}) \ud\Omega \,.
  \\
  &= \frac{\pi}{2} \vec{n} \vd \int_{\vec{\Omega}\vd\vec{n} < 0} \vec{\Omega}
  \vec{\Omega} f(\vec{x},\vec{\Omega}) \ud\Omega \,.
  \\ 
  \intertext{A reflecting boundary condition for $f$ causes this integral to
    be an even function of $\vec{\Omega}$:}
  \vec{d}(\vec{x})
  &= \frac{\pi}{4} \vec{n} \vd \int_{S}\vec{\Omega} \vec{\Omega}
  f(\vec{x},\vec{\Omega}) \ud\Omega
  \\
  &=  \frac{\pi}{4} \vec{n} \vd \Dtens \,.
\end{align*}
With this result, Eq.~\eqref{eq:flatAdBc} is the Marshak boundary condition:
...
\begin{equation*}
\frac{1}{c} \pder{\phi}{t} (\vec{x}, t)
  - \grad \vd \Dtens \vd \grad \phi(\vec{x}, t)
  + \sigma(\vec{x}) \phi(\vec{x}, t)
 = \sigma_s(\vec{x}) \phi(\vec{x},t) + q(\vec{x},t)\,.
\end{equation*}
...
\begin{equation*}
  \vec{\Omega}\vd \grad f(\vec{x}, \vec{\Omega})
  + \sigma(\vec{x}) f (\vec{x}, \vec{\Omega})
= \frac{1}{2\pi} \,, \quad 0 \le \omega < 2\pi \,.
\end{equation*}
