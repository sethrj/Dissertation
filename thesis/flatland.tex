...
$4\pi$
\cite{Zik1997}. Most well-constructed quadrature set will also correctly
integrate the spherical harmonic functions \cite{Rab2007}, which are linear
combinations of the angular moments of Table~\ref{tab:angularMoments}:
\begin{equation*}
  \gamma_n
...
polar angle $\mu=0$. In order to integrate the flatland moments
correctly, the combination of weights and angles to be chosen should correctly
integrate the angular moments $\gamma_n$, which instead of being spherical
harmonics are products of trigonometric functions:
\begin{equation*}
  \gamma_n
  = \sum_{m=1}^{M} \abs{ \sin \theta_m }^n w_m
  = \int_{2\pi} \abs{ \sin \theta }^n \ud\theta \,.
\end{equation*}
TODO chebyshev etc.

Additionally, to retain compatibility with the isotropic scattering
