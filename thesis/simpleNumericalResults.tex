...
The first test problems for the anisotropic diffusion boundary conditions test a
simple problem with incident boundary conditions.
...
\begin{figure}[tb]
  ...
  \Caption{Steady-state AD boundary condition test problem.}{
    ...
    region in the lower-left is a Gaussian source used in a later problem.}
...
formulate a problem with a spatially smooth source in the interior.
In this problem, the
primary ``sink'' for particles is by exiting one boundary several mean
free paths away from the source region.

\subsubsection{Problem description}

The problem has the same physical description as the previous: two diffusive
regions surrounding an optically thin channel. The boundary conditions are
...
\begin{equation*}
  q(x,y) = 2 \eexp^{-2 (x^2 + y^2) } \,.
\end{equation*}
This problem, with the Gaussian source, is depicted in
Fig.~\ref{Fig:bcReactorProblem}. The grid width used is $\Delta_x = \Delta_y =
0.1$.
...
To begin, we plot $f$ (Fig.~\ref{fig:bcReactorF}) at the top of the problem for the three alternative
boundary conditions for the purely absorbing transport problem. Only two---the white and reflecting
...
\begin{figure}[tb]
  ...
  \subfloat[$f(2.5,10,\omega)$]{%
   \hspace{-.5in}\input{adbc-presentation/adf-center-ymax/plot}\hspace{-.5in}%
    %\input{adbc-reactor/adf-center-ymax/plot}%
   \label{fig:bcReactorFchan}%
  }%
  \subfloat[$f(1.5,10,\omega)$]{%
   \hspace{-.5in}\input{adbc-presentation/adf-insideleftedge-ymax/plot}\hspace{-.5in}%
...
time-dependent situation: a linear, time-dependent problem with no material
...
Fig.~\ref{fig:tdReactorProblem}, meant to be a case in which the temporal and
