...
\begin{enumerate}
  \item \emph{Linearize the system.} Calculate the starting temperature $T^n$
    from the stored material energy density $U_m^n$. Using the temperature,
    calculate
  ...
  \item \emph{Update the material temperature.} Because of the linearization of
    ...
    $U_r^{n+1} \ne a (T^{n+1})^4$. To calculate the material
    temperature, we must first calculate the material energy using
    Eq.~\eqref{eq:matenConservationUpdate2}, and then invert the integral in
    Eq.~\eqref{eq:matEnergyDens} to solve for $T^{n+1}$ given $U_m^{n+1}$.
\end{enumerate}
