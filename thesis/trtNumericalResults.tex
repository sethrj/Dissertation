...
This test problem is not a ``classic'' Marshak wave: rather than a constant
incident radiation boundary, it contains a large pulse of energy as an initial
condition. Yet it does possess the same qualitative behavior as a Marshak wave,
as the pulse penetrates the colder material.
...
anticipated, the flux limiter is used less for anisotropic diffusion.
...
circled in Fig.~\ref{fig:doglegMaterials}, have the $(x,y)$ coordinates: $(0.1,
...
\begin{figure}[tbh]
  ...
  The logarithm of the opacity is shown in white, blue, and black, and a subset
...
The original problem also contains a material energy diffusion term (i.e.,
Eq.~\eqref{eq:explanMaterial} has an additional term $-\grad D_m \grad T$) which
we omit.
...
large voided regions: it should therefore lieoutside the comfort zone of
