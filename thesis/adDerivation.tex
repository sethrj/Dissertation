...
\end{equation}
where $I^i$ is the solution from the previous time step or a user-specified
initial
condition.
\end{subequations}
...
To formulate transport-matched boundary conditions---%
...
approximations. We make an ansatz that the spatial gradients
are weak, the speed of light is large, and the solution is mildly
(but not necessarily linearly) anisotropic:
\begin{align} \label{eq:ansatz}
  I &= O(1), &
  \sigmast &= O(1), &
  I - \frac{\phi}{4\pi} = \Psi &= O(\epsilon), &
  \grad &= O(\epsilon), &
  \frac1c &= O(\epsilon^2) \,.
\end{align}
These scalings are the same as in an asymptotic derivation of the standard
diffusion equation \cite{Mor2000}, where the intensity is expanded as
\begin{equation*}
  I \sim I^{(0)} + I^{(1)} \epsilon + I^{(2)} \epsilon^2 + \cdots\,.
\end{equation*}
That asymptotic analysis shows that in a diffusive problem, the leading-order
intensity is isotropic:
\begin{equation*}
  I^{(0)} = \frac{\phi}{4\pi}\,,
\end{equation*}
and the $O(\epsilon)$ term $I^{(1)}$ is anisotropic. The analysis therefore
implies $\phi=O(1)$ and $\vec{F}=O(\epsilon)$. With this ansatz, the transport
equation for $\Psi$ has the scaling
\begin{equation} \label{eq:capPsiVolScaled}
  \underbrace{\frac{1}{c} \pder{}{t}\Psi}_{O(\epsilon^3)}
+ \underbrace{\vec{\Omega}\vd \grad \Psi}_{O(\epsilon^2)}
+ \underbrace{\sigmast \Psi}_{O(\epsilon)}
  =
- \underbrace{\frac{1}{4\pi} \vec{\Omega}\vd \grad \phi}_{O(\epsilon)}
+ \underbrace{\frac{1}{4\pi} \grad \vd\vec{F}}_{O(\epsilon^2)}
+ \underbrace{\frac{1}{c} \Psi^i \delta(t)}_{O(\epsilon^3)} \,.
\end{equation}
Here, as in the derivation of the integral transport equation in \cite{Pri2010},
we have embedded the initial condition for the transport equation as a
delta-in-time source term. We may correspondingly write the integral transport
equation for $\Psi$ in the interior as
\begin{equation*}
  \Psi = 
  - \epsilon \lopinv{v}{\frac{1}{4\pi} \vec{\Omega}\vd \grad \phi}
  + \epsilon^2 \lopinv{v}{\frac{1}{4\pi} \grad \vd\vec{F} }
  + \epsilon^3 \lopinv{i}{\Psi^i}\,.
\end{equation*}
Rather than taking an asymptotic expansion of the differential transport
equation for $I$ to find a linear-in-angle form for $\Psi$, we apply the
scaling to the integral equation for $\Psi$ to find an anisotropic form.

Returning to the integral form of $\tilde\Psi$ in
Eq.~\eqref{eq:inverseTransportBrief}, where we have modified the boundary
condition terms, we see that
...
$O(\epsilon^3)$. Neglecting those terms in Eq.~\eqref{eq:inverseTransportBrief},
...
\end{equation*}
The leading order of $I$ is indeed isotropic. To analyze the anisotropic term,
we write the scaled transport solution for $f$ as
\begin{equation*}
  \epsilon \vec{\Omega}\vd \grad f + \sigma f = \frac{1}{4\pi}\,,
\end{equation*}
which shows $f=O(1)$.
Furthermore, because $f$ is continuous in space, the diffusion tensor $\Dtens$
is also continuous, and the solution for $\phi$ therefore has a smooth first
derivative,%
\footnote{
The smooth ``boundary layer'' at a material interface differs from standard
diffusion, which has ``kinks'' in $\phi$ but no boundary layer at discontinuous
material interfaces, where $D$ is
discontinuous. The exact solution to $I$ has both a ``kink'' and a boundary
layer.
}
a fact that meshes well with the supposition that spatial gradients of the
result are $O(\epsilon)$. 
Therefore, the anisotropic component $I-\frac{\phi}{4\pi}$ is $O(\epsilon)$ as
expected: it is the product of an $O(1)$ term, $f$, and an $O(\epsilon)$
term, $\grad \phi$.
