...
\\
  ...
  \eexp^{ -\tau(\vec{x}, \vec{x} - s_b \vec{\Omega})}
  U(ct - s_b) \,.
\\ \intertext{ Just like in the incident boundary situation, we apply the Taylor
...
-\left[f(\vec{x} - s_b\vec{\Omega}, \vec{\Omega}_r) \vec{\Omega}_r 
  \right]
  \eexp^{ -\tau(\vec{x}, \vec{x} - s_b \vec{\Omega})}
  U(ct - s_b) \vec{\Omega}_r \vd \grad \phi(\vec{x}, t) 
\\
&=
- \lopinv{b}{f(\vec{\Omega}_r)}_{\partial V_r}
\vec{\Omega}_r \vd \grad \phi(\vec{x}, t)
\\
&=
- \lopinv{b}{f(\vec{\Omega}_r)}_{\partial V_r}
...
\\
&=
- \lopinv{b}{f(\vec{\Omega}_r)}_{\partial V_r}\vec{\Omega}\vd \grad
\phi(\vec{x}, t)
\\
&\qquad+ 2(\vec{\Omega} \vd \vec{n})\lopinv{b}{f(\vec{\Omega}_r)}_{\partial V_r}
\vec{n} \vd \grad \phi(\vec{x}, t) \,.
\end{align*}
Equation~\eqref{eq:reflection} has been substituted and expanded to more
explicitly express $\vec{\Omega}_r$.

On a reflecting boundary, the exact intensity satisfies
$\vec{n} \vd \grad I = 0$.
This also implies $\vec{n} \vd \grad \phi=0$, which is a tremendously
convenient identity that simplifies the reflecting boundary approximation to
\begin{equation} \label{eq:reflApprox}
\lopinv{b}{\tilde\Psi(\vec{\Omega}_r)}_{\partial V_r}
\approx
- \lopinv{b}{f(\vec{\Omega}_r)}_{\partial V_r}\vec{\Omega}\vd \grad \phi(\vec{x}, t)\,.
\end{equation}
...
diffusion analysis. It allowed us to treat a specified boundary condition 

\horizsep

An important property of $\lopinv{b}{\cdot}$ is that, for
