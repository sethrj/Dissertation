...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Linear time-dependent problems}
...
Taking the first angular moment of Eq.~\eqref{eq:tdVolApprox2} gives the
...
\begin{equation}\label{eq:tdInitInit}
 ...
 = I^i(\vec{x},\vec{\Omega})\,.
\end{equation}
Furthermore, by construction we demand that its solution rapidly tends to zero
for increasing $t$:
...
\end{equation*}
To show this, we take the diffusive scaling that absorption is very small:
$\sigma_a \equiv \sigma - \sigma_s = O(\epsilon^2)$.
...
We demand that the boundary layer solution tend to zero for increasing distances
from the boundary:
...
The boundary condition is at this point incomplete, because the transport
...
\frac{1}{4\pi} \phi (\vec{x},t)
  ...
  \vd \grad \phi(\vec{x},t) \,.
\end{align*}
\emph{The above equation is not generally satisfied}. It only holds true when
$f$ is an ``even'' function of $\vec{\Omega}$, e.g.\ in an optically thick,
homogeneous material, where $f$ is isotropic. At material
boundaries, $f$ is strongly anisotropic, and the above relation is not
...
\end{equation}
In other words, the transport equation for $f$ should have boundary conditions
that enforce this relationship. For example, both specularly reflecting and
``white'' boundary conditions satisfy the condition in Eq.~\eqref{eq:hoBc}.
The above condition does not \emph{define} $f$; it only restricts it so that
...
is applicable to the linearized radiation equation.
...
\subsubsection{Behavior in voids}
...
\S\ref{sec:bgFld}). Standard diffusion fails catastrophically where $\sigma=0$,
but anisotropic diffusion gracefully yields a qualitatively realistic
...
on which $\Dtens$ has the greatest action) can be straightforwardly deduced by
...
varied only along the $x$ axis. From the discussion above, we may determine
\emph{a priori} that, even though the physical problem was a function of $(x,y)$
because of the problem's source term, the diffusion tensor had principle
eigenvectors oriented along the Cartesian axes, yielding $D^{xy}=D^{yx}=0$ and
allowing the use of a very simple spatial discretization.
...
anisotropic diffusion tensor, is also continuous.
...
must therefore correspond to a discontinuity in $\tpder\phi x$.
...
Yet the nature of this tensor product makes the interpretation of
...
It is an admittedly \emph{ad hoc} correction, but it restores a qualitative
...
equation in the interior under certain asymptotic assumptions: primarily, that
