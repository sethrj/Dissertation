...
symmetric about $\vec{n}$, then $f$ is only a function of the cosine angle
between $\vec{\Omega}$ and $\vec{n}$:
\begin{equation*}
f(\vec{\Omega}) = \hat f( \vec{\Omega} \vd \vec{n})\,.
\end{equation*}

If $\vec{n}$ is an eigenvector of $\Dtens$, then
\begin{equation*}
  \Dtens \vd \vec{n} = \lambda \vec{n}\,,
\end{equation*}
where $\lambda$ is a constant. Dotting Eq.~\eqref{eq:dDefinition} with
$\vec{n}$ and omitting the $\vec{x}$ parameter for brevity, we find
\begin{align*}
  \Dtens \vd \vec{n}
  &= \int_{4\pi} \vec{\Omega} \vec{\Omega} f(\vec{\Omega}) \ud\Omega \vd \vec{n}
  \\
  &= \int_{4\pi} (\vec{\Omega} \vd \vec{n}) \vec{\Omega} f(\vec{\Omega}) \ud\Omega
  \,.
  \\ 
  \intertext{If $f$ is azimuthal about $\vec{n}$, then}
  \Dtens \vd \vec{n}
  &= \int_{4\pi} (\vec{\Omega} \vd \vec{n}) \vec{\Omega}
    \hat f(\vec{\Omega} \vd \vec{n}) \ud\Omega \,.
  \\ 
  \intertext{We change the angular coordinates so that $\vec{i}=\vec{n}$, which
  implies $\vec{\Omega} \vd \vec{n}=\mu$:}
  \Dtens \vd \vec{n}
  &= \int_{0}^{2\pi} \int_{-1}^{1} (\mu)
  \left( \mu \vec{n} + \sqrt{1-\mu^2} \cos \theta \vec{u} + \sqrt{1-\mu^2} \sin
...
odd function of $\theta$, so the $\vec{u}$ and $\vec{v}$ components are zero.}
  \Dtens \vd \vec{n}
  &= 2\pi \int_{-1}^{1} \mu^2 \hat f(\mu) \ud\mu \vec{n}
  \\
  &= \lambda \vec{n} \,.
\end{align*}
Thus, if $f$ is azimuthally symmetric about $\vec{n}$, $\vec{n}$ is an
eigenvector of $\Dtens$.
