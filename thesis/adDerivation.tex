...
spatial gradients are scaled as $O(\epsilon)$, and the boundary layer
...
\begin{equation} \label{eq:ilVol}
 ...
 \phiil(\vec{x},\vec{\Omega}',t) \,,
\end{equation}
where
the zeroth angular moment of the initial layer solution:
\begin{equation*}
  \phiil(\vec{x},t) \equiv \int_{4\pi} \Iil^{0}(\vec{x},\vec{\Omega},t) \ud
  \Omega \,.
\end{equation*}
Because the initial layer is defined for times close to $t=0$, it accounts for
the initial
...
The asymptotic analysis begins by expand $\Iil$ in powers of $\epsilon$,
\begin{equation*}
  ...
  + \cdots\,,
\end{equation*}
and substituting into the scaled transport equation. The zeroth moment of the
...
\begin{equation*}
  ...
  = \frac{1}{4\pi} \phi^{0} + \epsilon \left[ \frac{1}{4\pi} \phi^{1}
    - f \vec{\Omega} \vd \grad \phi^{0} \right] + \cdots\,.
\end{equation*}
...
To match the initial layer solution to the interior solution, each power of
$\epsilon$ must match. To satisfy Eq.~\eqref{eq:ilLimit}, each must also rapidly
tend to zero. We begin with the $\epsilon^0$ terms in the initial layer
...
\begin{equation*}
  \phiil^0(0) = 0\,.
\end{equation*}
To use this fact, we take the leading order terms of initial condition,
Eq.~\eqref{eq:ilInit2}:
...
\end{equation*}
Using $\phiil^0(0) = 0$, we have a relation between the leading order scalar
...
Because $\phiil^0(0) = 0$, we can exactly solve for the $O(1)$ component of
$\Iil$ in Eq.~\eqref{eq:ilVolZeroth} for all times:
\begin{equation*}
  \frac{1}{c}\pder{\Iil^0}{t} + \sigma \Iil^0 = 0
  \lra \Iil^0(t) = \Iil^0(0) \eexp^{-\sigma c t}
  = \left[ \I^i - \frac{\phi^i}{4\pi} \right] \eexp^{-\sigma c t}\,.
\end{equation*}
Now we turn to the $O(\epsilon)$ component of 
