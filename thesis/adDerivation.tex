...
\begin{equation} \label{eq:fullTransportBndy}
 ...
 \quad \vec{x} \in \partial V_b, \ \vec{\Omega} \vd \vec{n} < 0,\ 0 \le t \le \Delta_t
\end{equation}
...
\begin{equation} \label{eq:fullTransportRefl}
  ...
  \,,
 \quad \vec{x} \in \partial V_r, \ \vec{\Omega} \vd \vec{n} < 0,\ 0 \le t \le \Delta_t
\end{equation}
...
$\vec{F}$ that only depends on the low-order unknown $\phi$ and some other
function of space and angle that does not depend on $I$ or its moments.
...
\left(  \vec{\Omega} - 2(\vec{\Omega} \vd \vec{n}) \vec{n} \right)
  ...
  \phi(\vec{x}_b,t)}
  + \lopinv{b}{f(\vec{x}_b,\vec{\Omega}) 2(\vec{\Omega} \vd \vec{n}) \vec{n} \vd \grad \phi(\vec{x}_b,t) } \,.
\\ 
\intertext{On a reflecting boundary at any point $\vec{x}_b$, the exact intensity satisfies $\vec{n} \vd
...
\begin{equation*}
  4 \vec{F}^-(\vec{x}, t) = \phi(\vec{x}, t) + 2  D(\vec{x}) \vec{n} \vd \grad \phi(\vec{x}, t)\,.
\end{equation*}
...
where $I$ is a complex function of angle. Chapters~\ref{chap:simpleNumerical}
