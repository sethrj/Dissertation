...
$0 \le t \le \Delta_t$,
with opacities frozen at some value $\sigmast$ (which could be
evaluated at the initial material temperature $T(0)$ for a semi-implicit
formulation, or at some estimate $T(\Delta_t)$ if the nonlinearities are being
converged).
The transport equation for the radiation intensity $I$ is:
...
An alternative condition can be derived that also assumes $f$ is azimuthally
