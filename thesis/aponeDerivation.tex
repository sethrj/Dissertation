...
in
the homogeneous case, it reduces to the standard \Pone\ (spherical harmonic)
approximation. 
...
approximation. The boundary and
...
The derivation diverges because the \APone\
...
in Eq.~\eqref{eq:capPsiVol} must be retained to preserving $O(\epsilon^2)$
...
\begin{equation}\label{eq:ap1ficksLaw1}
  ...
  \vd \grad \phi(\vec{x}, t) \,,
\end{equation}
or, multiplying through by $\sigma$ and moving the right-hand side,
...
equation~\eqref{eq:loVol}; $\Iv$ in the interior is thus approximated by
...
Eq.~\eqref{eq:tdSuperposition}, the initial conditions of the original transport
...
\end{equation}
Substituting this into the previous equation and multiplying by 2, we arrive at
...
anisotropic Fick's law of Eq.~\eqref{eq:anisotropicFicks}:
\begin{equation*}
 \vec{F}(\vec{x},t)
  = -\varsigma(\vec{x}) \Dtens(\vec{x}) \vd \grad \phi(\vec{x}, t) \,. 
\end{equation*}
...
perform better in problems with rapid transient behavior.
