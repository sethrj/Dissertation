...
small term is $\grad \vd\vec{F} = O(\epsilon^2)$. Discarding it,
Eq.~\eqref{eq:capPsiVol} is
\begin{equation*}
  \left[ \frac{1}{c}\pder{}{t}
  + \vec{\Omega}\vd \grad
  + \sigma \right]
...
\end{equation}
Here we have used the definition of the anisotropic diffusion tensor from
...
We make the following substitution, which is $O(\epsilon^2)$ in the diffusive
limit:
\begin{align*}
  \frac{1}{\sigma(\vec{x})}
...
\begin{equation}\label{eq:ap1ficksLaw2}
  \frac{1}{c}\pder{\vec{F}}{t}(\vec{x},t) + \varsigma(\vec{x})\vec{F}(\vec{x},t) 
  = - \varsigma(\vec{x}) \Dtens(\vec{x}) \vd \grad \phi(\vec{x}, t) \,.
\end{equation}

This closure for $\vec{F}$ is used in conjunction with the low-order particle
conservation equation in the interior, Eq.~\eqref{eq:loVol}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In a homogeneous medium, the \APone\ equation limits to the \Pone\ equation.
