...
two-dimensional Cartesian mesh, a simple testbed environment where all cells are
...
roughly three unknowns per cell.) Instead, we derive some simple, conservative
...
discretization: a
``staggered mesh'' where the scalar intensity $\phi$ is cell-centered and
the exiting radiation flux $\vec{F}\vd \vec{n}$ is
...
The first step in a finite difference scheme is to assume that the unknown---%
...
\end{equation}
The unknown face-averaged radiation flux $F$---also known as leakage---and the
...
\end{equation}
The anisotropic \Pone\ approximation contains an additional term that accounts
for stringer time dependence:
...
leakage.
In certain simple problems, however, this is not an approximation. As long as the
...
very slight modification of common cell-centered diffusion scheme: the
...
approximate the transverse leakage through a face normal to the $x$ axis.
...
\begin{alignat*}{3}
 ...
 -\tfrac{1}{2}D^{xy} & \phi_{i+11,j+1}\quad
\\
             -D^{yy} & \phi_{i-1,j} &
...
anisotropic \Pone\ equations. The first simple discretization, where transverse
leakage is neglected, is only a minor modification of existing cell-centered
