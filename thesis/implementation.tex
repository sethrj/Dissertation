...
\begin{equation}\label{eq:ap1FicksImplicit}
  \vec{F}(\vec{x}) = - [1 - f(\vec{x})]\Dtens(\vec{x}) \vd \grad \phi(\vec{x})
  + f(\vec{x}) \hat{\vec{F}}(\vec{x})\,,
\end{equation}
where $\vec{F} = \vec{F}(t^{n+1})$, $\hat{\vec{F}}=\vec{F}(t^n)$, and
\begin{equation}\label{eq:ap1ImplicitF}
  f(\vec{x}) \equiv \frac{1}{\varsigma(\vec{x}) c \Delta_t}\,.
\end{equation}
\end{subequations}
Setting $f=0$ reduces this equation to Eq.~\eqref{eq:adFicks2d}.
...
Setting $f=0$ results in the simpler anisotropic diffusion representation:
...
\begin{equation*}
  F_{i+1/2}^{x} =
  - (1 - f_{i,j})D_{i,j}^{xx} \frac{\phi_* - \phi_{i}}{\Delta_{x,i}/2}
  + f_{i,j} \hat{F}_{i+1/2}^{x} \,.
\end{equation*}
Evaluating the leakage on same face from cell $i+1,j$, we obtain:
\begin{equation*}
  F_{i+1/2}^{x} =
  - (1 - f_{i+1,j})D_{i+1,j}^{xx} \frac{\phi_{i+1,j} - \phi_*}{\Delta_{x,i+1}/2}
  +  f_{i+1,j} \hat{F}_{i+1/2}^{x} \,.
\end{equation*}
Multiplying the equations by $\Delta_{x}/ [2 (1 - f)D^{xx}]$, we eliminate the
temporary $\phi_*$:
...
introducing a temporary cell-edge $\phi_*$ with which we approximate the
gradient,
\begin{align} \nonumber
  F_R^y 
  &= - D_{i,j}^{xx} \frac{\phi_* - \phi_{i,j}}{\Delta_{x,i} / 2}
...
\label{eq:discreteBcFBrief}
  F
  &= - D \frac{\phi_* - \phi}{\Delta / 2}
  + \gamma D \hat F \,,
\end{align}
where $\phi_*$ is the cell-edge scalar intensity, and $\hat F$ is the initial
...
\begin{equation*}
  \beta
  = \phi_*
  + \alpha D \frac{\phi_* - \phi}{\Delta / 2}
  - \alpha \gamma D \hat F \,.
\end{equation*}
A little manipulation gives
\begin{equation*}
  \phi_* = \left( 1 + \frac{\alpha D}{\Delta/2} \right)\inv \left( \beta
  + \frac{\alpha D}{\Delta/2} \phi + \alpha \gamma D \hat F
  \right)\,.
\end{equation*}
We use this result to eliminate $\phi_*$ from Eq.~\eqref{eq:discreteBcFBrief}:
\begin{align*}
  F
  &= - \frac{D}{\Delta / 2} \phi_* + \frac{D}{\Delta / 2}\phi
  + \gamma D \hat F
  \\
  F
...
\nonumber
F &= D \frac{\phi_* - \phi}{\Delta / 2} - \gamma D \hat F \,.
\end{align}
...
\begin{equation*}
  \beta = \phi_*
  + \alpha D \frac{\phi_* - \phi}{\Delta / 2} + \alpha \gamma D \hat F \,.
\end{equation*}
Eliminating $\phi_*$ from the previous equation gives the net current on the
left face:
\begin{equation*}
  F
  = -\left(  \frac{\Delta}{2 D} + \alpha\right)\inv \beta
