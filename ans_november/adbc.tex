...
Eq.~\eqref{eq:approxPsi3} shows that for that truth to hold,
\begin{equation}\label{eq:zetaCondition}
  ...
  \ud\Omega \,.
\end{equation}
One way to satisfy this is to make $f$ an even function of $\vec{\Omega}$, $\zeta(\vec{\Omega})=f(-\vec{\Omega})$. If
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Discussion}
The form we chose for $\zeta$ allowed the boundary condition on $\tilde\Psi$ to
be embedded in the anisotropic diffusion tensor $\Dtens$. It results in a
...
was not the only possible choice. A white boundary will also satisfy it,
...
By interpreting the AD method as a modification to $\psi-\frac{1}{4\pi}\phi$,
