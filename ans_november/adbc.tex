...
an ``internal" solution $\tilde\Psi$ and a ``boundary layer" solution
$\Psi_\mathrm{bl}$:
...
\begin{equation} \label{eq:tCapPsiVol}
  ...
  \frac{1}{4\pi} \vec{\Omega}\vd \grad \phi(\vec{x})\,.
\end{equation}
We define incident boundary conditions for this internal solution
to be
...
The boundary layer solution complements the internal solution, but is expected
